...
class IntelAmxMatrixMultiply {
   ...
   public:
    ...
    void MatrixMultiply(std::vector<Matrix<InputType>> &VA0, std::vector<Matrix<InputType>> &VA1,
                        std::vector<Matrix<InputType>> &VB0, std::vector<Matrix<InputType>> &VB1,
                        Matrix<OutputType> &C00, Matrix<OutputType> &C01, Matrix<OutputType> &C10,
                        Matrix<OutputType> &C11) {
...
void run_test(int thread_id, int iterations, TestData &data, double &result_time,
              double &result_gflops) {
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iterations; i++) {
        data.multiply.MatrixMultiply(data.VA0, data.VA1, data.VB0, data.VB1, data.C00, data.C01,
                                     data.C10, data.C11);
    }
...
int main() {
    ...
    for (int t = 0; t < thread_count; ++t) {
        for (int i = 0; i < 16; ++i) {
            thread_data[t].VA0.emplace_back(rows, cols);
            thread_data[t].VA0.back().Fill(2);
            thread_data[t].VB0.emplace_back(rows, cols);
            thread_data[t].VB0.back().Fill(2);
            thread_data[t].VA1.emplace_back(rows, cols);
            thread_data[t].VA1.back().Fill(2);
            thread_data[t].VB1.emplace_back(rows, cols);
            thread_data[t].VB1.back().Fill(2);
        }
    ...
    for (int t = 0; t < thread_count; ++t) {
        ...
        std::cout << "执行时间: " << std::fixed << std::setprecision(4) << times[t]
                  << " 秒, 性能: " << std::fixed << std::setprecision(4) << gflops[t]
                  << " GFLOPS\n";
    }
